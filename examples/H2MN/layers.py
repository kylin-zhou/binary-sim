...
class HyperedgeConv(MessagePassing):
    ...
    def forward(self, x, hyperedge_index, hyperedge_weight=None):
        ...
        self.flow = "target_to_source"
        out = self.propagate(
            hyperedge_index, x=out, norm=D, size=(num_edges, num_nodes)
        )
...
class HyperedgePool(MessagePassing):
    ...
    def forward(self, x, batch, edge_index, edge_weight):
        ...
        if edge_weight is None:
            ...
            D = scatter_add(
                edge_weight[edge_index[1]], edge_index[0], dim=0, dim_size=x.size(0)
            )
        D = 1.0 / D
        D[D == float("inf")] = 0
        ...
        batch = batch[perm]
        edge_index, edge_attr = self.filter_hyperedge(
            edge_index, edge_weight, perm, num_nodes=score.size(0)
        )
...
class CrossGraphConvolutionOperator(MessagePassing):
    ...
    def forward(self, x, assign_index, N, M):
        ...
        target_x_denominator = torch.sqrt(
            torch.sum(torch.square(target_x), dim=-1) + 1e-6
        )
        global_x_denominator = torch.sqrt(
            torch.sum(torch.square(global_x), dim=-1) + 1e-6
        )
...
class CrossGraphConvolution(torch.nn.Module):
    ...
    def forward(self, x_left, batch_left, x_right, batch_right):
        num_nodes_x_left = scatter_add(
            batch_left.new_ones(x_left.size(0)), batch_left, dim=0
        )
        shift_cum_num_nodes_x_left = torch.cat(
            [num_nodes_x_left.new_zeros(1), num_nodes_x_left.cumsum(dim=0)[:-1]], dim=0
        )
        cum_num_nodes_x_left = num_nodes_x_left.cumsum(dim=0)

        num_nodes_x_right = scatter_add(
            batch_right.new_ones(x_right.size(0)), batch_right, dim=0
        )
        shift_cum_num_nodes_x_right = torch.cat(
            [num_nodes_x_right.new_zeros(1), num_nodes_x_right.cumsum(dim=0)[:-1]],
            dim=0,
        )
        cum_num_nodes_x_right = num_nodes_x_right.cumsum(dim=0)

        adj = torch.zeros(
            (x_left.size(0), x_right.size(0)), dtype=torch.float, device=x_left.device
        )
        # Construct batch fully connected graph in block diagonal matirx format
        for idx_i, idx_j, idx_x, idx_y in zip(
            shift_cum_num_nodes_x_left,
            cum_num_nodes_x_left,
            shift_cum_num_nodes_x_right,
            cum_num_nodes_x_right,
        ):
        ...
        out1 = self.cross_conv(
            (x_right, x_left), assign_index1, N=x_right.size(0), M=x_left.size(0)
        )

        assign_index2 = torch.stack([row, col], dim=0)
        out2 = self.cross_conv(
            (x_left, x_right), assign_index2, N=x_left.size(0), M=x_right.size(0)
        )
...
class MLP(torch.nn.Module):
    def __init__(self, args):
        ...
        self.lin0 = torch.nn.Linear(self.args.nhid * 2 * 2, self.args.nhid)
        nn.init.xavier_uniform_(self.lin0.weight.data)
        nn.init.zeros_(self.lin0.bias.data)

        self.lin1 = torch.nn.Linear(self.args.nhid, 1)
        nn.init.xavier_uniform_(self.lin1.weight.data)
        nn.init.zeros_(self.lin1.bias.data)
    ...
    def forward(self, scores):
        scores = F.relu(self.lin0(scores))
        scores = F.dropout(scores, p=self.args.dropout, training=self.training)
        scores = torch.sigmoid(self.lin1(scores)).view(-1)
...
class AttentionModule(torch.nn.Module):
    ...
    def setup_weights(self):
        ...
        Defining weights.
        """
        self.weight_matrix = torch.nn.Parameter(torch.Tensor(self.nhid, self.self.nhid))
