...
class GraphEmbeddingDataset(object):
    ...
    def collate(self, data_list):
        batch = Batch.from_data_list(
            [data[0] for data in data_list] + [data[1] for data in data_list]
        )
        return batch
...
class FuncDataset(Dataset):
    ...
    def get_graph_pair(self, graphset):
        ...
        while True:
            ...
            # if cfg diff > 0.5*min, repeat sample
            if np.absolute(len(origin_graph) - len(pos_graph)) < int(
                0.5 * min(len(origin_graph), len(pos_graph))
            ):
                count += 1
                break
            # force quit
...
class GraphClassificationDataset(object):
    ...
    def collate(self, data_list):
        batchS = Batch.from_data_list(
            [data[0] for data in data_list] + [data[0] for data in data_list]
        )
        batchT = Batch.from_data_list(
            [data[1] for data in data_list] + [data[2] for data in data_list]
        )
        batchL = [1 for data in data_list] + [0 for data in data_list]
        return batchS, batchT, batchL

    def create_batches(self, funcs, collate, shuffle_batch=True):
        data = FuncPairDataset(funcs)
        loader = torch.utils.data.DataLoader(
            data,
            batch_size=self.args.batch_size,
            shuffle=shuffle_batch,
    ...
    def transform(self, data):
        ...
        new_data["target"] = torch.from_numpy(np.array(data[2], dtype=np.float32)).to(
            self.args.device
        )
        return new_data
...
class GraphRegressionDataset(object):
    ...
    def process_dataset(self):
        ...
        self.training_set, self.val_set = random_split(
            self.training_graphs, [train_num, val_num]
        )
    ...
    def create_batches(self, graphs):
        ...
        source_loader = DataLoader(
            graphs, batch_size=self.args.batch_size, shuffle=True
        )
    ...
    def transform(self, data):
        ...
        norm_ged = self.nged_matrix[
            data[0]["i"].reshape(-1).tolist(), data[1]["i"].reshape(-1).tolist()
        ].tolist()
        new_data["target"] = (
            torch.from_numpy(np.exp([(-el) for el in norm_ged]))
            .view(-1)
            .float()
            .to(self.args.device)
        )
...
def hypergraph_construction(edge_index, edge_attr, num_nodes, k=2, mode="RW"):
    if mode == "RW":
        ...
        adj = torch.zeros(
            (num_nodes, num_nodes), dtype=torch.float, device=edge_index.device
        )
        ...
        if k == 1:
            ...
            edge_index, edge_attr = add_remaining_self_loops(
                hop_edge_index, hop_edge_attr, num_nodes=num_nodes
            )
...
def hyperedge_representation(x, edge_index):
    ...
    coef = softmax(
        torch.sum(x_rep * gloabl_edge_rep, dim=1),
        edge_index[1],
        num_nodes=x_rep.size(0),
    )
