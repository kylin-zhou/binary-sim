    ...
    if osp.exists(pyg_file + ".nodes.pyg") == False:
        ...
        with open(node_file, "r") as rf:
            while True:
                ...
                if node_type == "item":
                    if len(info[2]) < 50:
                        node_embeds[node_type][node_id_v2] = np.zeros(
...
def format_pyg_graph(edge_file, node_file, pyg_file, label_file=None):
    if osp.exists(pyg_file + ".pt") and args.reload == False:
        ...
        print(
            "PyG graph of "
            + ("session2" if "session2" in pyg_file else "session1")
            + " has generated"
        )
        return 0
    else:
        print("##########################################")
        print(
            "### Start generating PyG graph of "
            + ("session2" if "session2" in args.storefile else "session1")
        )
        print("##########################################\n")
    ...
    for edge_type in edges:
        ...
        graph[(source_type, edge_type, dest_type)].edge_index = torch.vstack(
            [source, dest]
        )
    ...
    print("##########################################")
    print(
        "### Complete generating PyG graph of "
        + ("session2" if "session2" in args.storefile else "session1")
    )
    print("##########################################")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Whether node features should be reloaded",
    )
