import collections
from dataset.cfg_dataset import (
    GraphEditDistanceDataset,
    TestGraphEditDistanceDataset,
)
...
def build_datasets(config):
    ...
    if config["data"]["problem"] == "graph_edit_distance":
        training_validation_set = GraphEditDistanceDataset()
    else:
        raise ValueError("Unknown problem type: %s" % config["data"]["problem"])
    return training_validation_set
...
def build_test_datasets(config):
    ...
    if config["data"]["problem"] == "graph_edit_distance":
        test_set = TestGraphEditDistanceDataset()
    else:
        raise ValueError("Unknown problem type: %s" % config["data"]["problem"])
    return test_set
