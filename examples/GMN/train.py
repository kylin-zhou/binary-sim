...
def test():
    ...
    with torch.no_grad():
        ...
        accumulated_pair_precision = []
        for _ in tqdm(range(config["training"]["n_valid_steps"])):
            ...
            pred_sim, pair_precision, pair_auc = auc(similarity, labels)
            accumulated_pair_precision.append(pair_precision)
            accumulated_pair_auc.append(pair_auc)
            # if np.mean(accumulated_pair_auc) > 0.99:
            #     print(labels, pred_sim)

        eval_metrics = {
            "pair_precision": np.mean(accumulated_pair_precision),
...
# load dataset
train_valid_set = build_datasets(config)

if config["training"]["mode"] == "pair":
    training_data_iter = train_valid_set.pairs(config["training"]["batch_size"])
    first_batch_graphs, _ = next(training_data_iter)
else:
    training_data_iter = train_valid_set.triplets(config["training"]["batch_size"])
    first_batch_graphs = next(training_data_iter)

valid_data_iter = train_valid_set.pairs(config["evaluation"]["batch_size"], valid=True)
...
for i_iter in range(config["training"]["n_training_steps"]):
    ...
    # evaluation
    if i_iter % config["training"]["print_after"] == 0:
        ...
        if (
            i_iter
            // config["training"]["print_after"]
            % config["training"]["eval_after"]
            == 0
        ):

            print("valid")
            eval_metrics = test()
            pair_auc = eval_metrics["pair_auc"]

            if pair_auc > best_metric:
                torch.save(model, "checkpoint/embed_model.pt")
                best_metric = pair_auc
                count = 0
            # else:
            #     count += 1
            #     if count > 3:
            #         print(f"early stopping at epoch: {i_iter}")
            #         break
